...
if ($run == 'content.php') {
    ...
    if ($action == 'add') {
        $key = hm_get('key');
        $include = get_option(['section' => 'content_slider', 'key' => 'include_content_' . $key]);
        if ($include == 'yes') {
            $display_box = true;
        }
    } else if ($action == 'edit') {
...
{
    $id = get_id();
    $content_slider_image = get_con_val(['name' => 'content_slider_image', 'id' => $id]);
    $content_slider_image = json_decode($content_slider_image, true);
    $content_slider_title = get_con_val(['name' => 'content_slider_title', 'id' => $id]);
    $content_slider_title = json_decode($content_slider_title, true);
    $content_slider_description = get_con_val(['name' => 'content_slider_description', 'id' => $id]);
    $content_slider_description = json_decode($content_slider_description, true);
    $content_slider_link = get_con_val(['name' => 'content_slider_link', 'id' => $id]);
    $content_slider_link = json_decode($content_slider_link, true);

    if (is_array($content_slider_image)) {

        $jquery_plugin = get_option(['section' => 'content_slider', 'key' => 'jquery_plugin', 'default_value' => 'galleria']);
        if ($jquery_plugin == 'galleria') {

            $galleria_height = get_con_val(['name' => 'galleria_height', 'id' => $id]);
            if (!is_numeric($galleria_height) OR $galleria_height == 0) {
                $galleria_height = get_option(['section' => 'content_slider', 'key' => 'galleria_height', 'default_value' => '600']);
            }
        ...
        } else if ($jquery_plugin == 'fancybox') {
            ...
            $default = "'transitionIn'	:	'elastic', " . "\n" .
                ...
                "'speedOut'	:	200, " . "\n" .
                "'overlayShow'	:	false " . "\n";
            echo get_option(['section' => 'content_slider', 'key' => 'fancybox_setting', 'default_value' => $default]);
            echo '});' . "\n\r";
            echo '});' . "\n\r";
            echo ' </script>' . "\n\r";
