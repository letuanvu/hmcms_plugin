...
function nivoslider_sliders()
{
    ...
    $data = [];
    $tableName = DB_PREFIX . "object";
    $whereArray = ['key' => MySQL::SQLValue('nivoslider')];
...
function nivoslider_slide_setting()
{
    ...
    if (isset($_POST['add_slide'])) {
        ...
        if (is_numeric($image)) {
            ...
            $slider = get_option(['section' => 'nivoslider', 'key' => $slider_id . '_slides']);
            ...
            $new_order = $bigest + 1;
            $slider[$new_order] = ['image' => $image, 'link' => $link, 'target_blank' => $target_blank];
...
{
    $slide_theme = get_option(['section' => 'nivoslider', 'key' => $slider_id . '_slide_theme', 'default_value' => 'hmcms']);
    $slider = get_option(['section' => 'nivoslider', 'key' => $slider_id . '_slides']);
    $slider = json_decode($slider, true);
    if (!is_array($slider)) {
        $slider = [];
...
function nivoslider_display_asset()
{
    ...
    $custom_script = get_option(['section' => 'nivoslider', 'key' => $slider_id . '_custom_script']);
    $slide_theme = get_option(['section' => 'nivoslider', 'key' => $slider_id . '_slide_theme', 'default_value' => 'hmcms']);
    ...
    if ($custom_script != '') {
        ...
        foreach ($setting as $op_key => $op_val) {
            $default_value = get_option(['section' => 'nivoslider', 'key' => $slider_id . '_' . $op_key]);
            if ($default_value == '') {
