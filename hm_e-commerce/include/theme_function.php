...
function hme_in_cart($pid){
	if(!isset($_SESSION['hmecart'])){$_SESSION['hmecart']=array();}
...
function hme_submit_cart(){
	if(isset($_SESSION['hmecart']) AND is_array($_SESSION['hmecart'])){
		...
		if(is_numeric($insert_id)){
			...
			$noti_email = get_option( array('section'=>'hme','key'=>'noti_email','default_value'=>'') );
...
function hme_customer_login_action($args){
	...
	$whereArray=array('email'=>MySQL::SQLValue($email));
	...
	if($name!='' AND $email!='' AND $mobile!='' AND $password_req!=FALSE){
		if($password == $password_again){
			...
			if($check_email){
				...
				/** login */
				hme_customer_login(array('email'=>$email,'password'=>$password));
