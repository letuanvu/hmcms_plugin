...
/* 
Đăng ký trang chi tiết đơn hàng
*/
$args=array(
		...
		case 'edit':
			...
			if( $hmdb->HasRecords() ){
				...
				if(isset($_POST['save_order_setting'])){
					...
					$whereArray=array('id'=>MySQL::SQLValue($oid));
				...
				hm_include(BASEPATH.'/'.HM_PLUGIN_DIR.'/hm_e-commerce/layout/edit_order.php',array('customer'=>$customer,'order_item'=>$order_item));
			}else{
		...
		case 'delete':
			...
			if( $hmdb->HasRecords() ){
				...
				if(isset($_POST['delete_order'])){
					...
					$whereArray=array('order_id'=>MySQL::SQLValue($oid));
					$hmdb->DeleteRows($tableName, $whereArray);
					
					$tableName = DB_PREFIX.'field';
					$whereArray=array('object_id'=>MySQL::SQLValue($oid),'object_type'=>MySQL::SQLValue('hme_order'));
					$hmdb->DeleteRows($tableName, $whereArray);
				...
				hm_include(BASEPATH.'/'.HM_PLUGIN_DIR.'/hm_e-commerce/layout/delete_order.php',array('customer'=>$customer,'order_item'=>$order_item));
			}else{
				echo '<div class="alert alert-danger" role="alert">'.hme_lang('orders_do_not_exist').'</div>';
			}
		break;
