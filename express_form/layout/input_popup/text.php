<div class="express_form_popup express_form_popup_text">
    ...
    build_input_form($args);
    ?>
    <button class="btn btn-default add_form_input_text">Thêm biểu mẫu vào form</button>
...
<script>
    $(document).ready(function () {
        /* input text */
        $('.express_form_popup_text input[name=label]').change(function () {
            var url = '?run=content_ajax.php&action=ajax_slug';
            var val = $(this).val();
            $.post(url, {val: val, accented: 'false', object: '0'}, function (data) {
                $('.express_form_popup_text input[name=name]').val(data);
            });
        });
        $('.add_form_input_text').click(function () {

            var input_attr = '';
            var label = $('.express_form_popup_text input[name=label]').val();
            var name = $('.express_form_popup_text input[name=name]').val();
            var value = $('.express_form_popup_text input[name=value]').val();
            var maxlength = $('.express_form_popup_text input[name=maxlength]').val();
            var placeholder = $('.express_form_popup_text input[name=placeholder]').val();
            var cssclass = $('.express_form_popup_text input[name=class]').val();
            var id = $('.express_form_popup_text input[name=id]').val();
            if (name != '') {
            ...
            if ($('.express_form_popup_text input[name=required]').is(':checked')) {
            ...
            var line = label + "\n" + '[shortcode=express_form&type=text' + input_attr + "]\n";
