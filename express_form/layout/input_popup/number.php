<div class="express_form_popup express_form_popup_number">
    ...
    build_input_form($args);
    ?>
    <button class="btn btn-default add_form_input_number">Thêm biểu mẫu vào form</button>
...
<script>
    $(document).ready(function () {
        /* input number */
        $('.express_form_popup_number input[name=label]').change(function () {
            var url = '?run=content_ajax.php&action=ajax_slug';
            var val = $(this).val();
            $.post(url, {val: val, accented: 'false', object: '0'}, function (data) {
                $('.express_form_popup_number input[name=name]').val(data);
            });
        });
        $('.add_form_input_number').click(function () {

            var input_attr = '';
            var label = $('.express_form_popup_number input[name=label]').val();
            var name = $('.express_form_popup_number input[name=name]').val();
            var value = $('.express_form_popup_number input[name=value]').val();
            var maxlength = $('.express_form_popup_number input[name=maxlength]').val();
            var placeholder = $('.express_form_popup_number input[name=placeholder]').val();
            var cssclass = $('.express_form_popup_number input[name=class]').val();
            var id = $('.express_form_popup_number input[name=id]').val();
            if (name != '') {
            ...
            if ($('.express_form_popup_number input[name=required]').is(':checked')) {
            ...
            var line = label + "\n" + '[shortcode=express_form&type=number' + input_attr + "]\n";
