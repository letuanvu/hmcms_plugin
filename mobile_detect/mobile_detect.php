...
function mobile_detect_main_setting()
{
    ...
    hm_include(BASEPATH . '/' . HM_PLUGIN_DIR . '/mobile_detect/layout/main_setting.php', ['themes' => $themes]);
...
function change_activated_theme($now_theme)
{
    if (!defined(IN_ADMIN)) {
        ...
        $domain_mobile = get_option(['section' => 'mobile_detect', 'key' => 'domain_mobile', 'default_value' => '']);
        $domain_theme = get_option(['section' => 'mobile_detect', 'key' => 'domain_theme', 'default_value' => '']);
        $ip_mobile = get_option(['section' => 'mobile_detect', 'key' => 'ip_mobile', 'default_value' => '']);
        $ip_theme = get_option(['section' => 'mobile_detect', 'key' => 'ip_theme', 'default_value' => '']);
        ...
        if (in_array($remote_addr, $ip_mobile_array)) {
        ...
        } else if (($domain == $domain_mobile) AND $domain_theme != '') {
            if (isset($_SESSION['mobile_detect_fixed_theme'])) {
                return $_SESSION['mobile_detect_fixed_theme'];
            } else {
                return $domain_theme;
            ...
            if (isset($_SESSION['mobile_detect_fixed_theme'])) {
                ...
                if ($detect->isMobile()) {
                    $theme = get_option(['section' => 'mobile_detect', 'key' => 'isMobile', 'default_value' => '']);
                }
                if ($detect->isTablet()) {
                    $theme = get_option(['section' => 'mobile_detect', 'key' => 'isTablet', 'default_value' => '']);
                }
                if ($detect->isiOS()) {
                    $theme = get_option(['section' => 'mobile_detect', 'key' => 'isiOS', 'default_value' => '']);
                }
                if ($detect->isAndroidOS()) {
                    $theme = get_option(['section' => 'mobile_detect', 'key' => 'isAndroidOS', 'default_value' => '']);
                }
            }
