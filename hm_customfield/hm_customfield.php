...
function hm_customfield_ajax_content($name='field_append_content',$checked=array()){
	...
	foreach($hmcontent->hmcontent as $item){
		...
		if(!isset($item['content_name'])){ $item['content_name'] = NULL; }
		if(!isset($item['content_key'])){ $item['content_key'] = NULL; }
...
function hm_customfield_ajax_taxonomy($name='field_append_taxonomy',$checked=array()){
...
function hm_customfield_ajax_field_type($type,$default=FALSE,$input_option=FALSE){
	...
	}elseif(in_array($type,$has_option)){
	...
	}elseif(in_array($type,$upload_file)){
	...
	}elseif(in_array($type,$upload_multi_file)){
	...
	while( $row=$hmdb->Row() ){
		...
		if(hm_get('action') == 'edit'){
			if(hm_get('run') == 'taxonomy.php'){
				$default_value = get_tax_val(array('name'=>$field_key,'id'=>hm_get('id')));
			}elseif(hm_get('run') == 'content.php'){
				$default_value = get_con_val(array('name'=>$field_key,'id'=>hm_get('id')));
			}
		}
		...
		if(in_array($field_type,$has_option)){
			...
			foreach($ex as $item){
				...
				$input_option[] = array('value'=>$ex_option[0],'label'=>$ex_option[1]);
			}
