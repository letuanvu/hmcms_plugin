...
while ($row = $hmdb->Row()) {
    ...
    switch ($object_type) {
        case 'content':
            ...
            if ($include_to_sitemap != 'no') {
                $include_to_sitemap = 'yes';
            }
            $status = get_con_val("name=status&id=$object_id");
            if ($status != 'public') {
                $include_to_sitemap = 'no';
            }
            ...
            if ($sitemap_change_frequency == 'auto' OR !$sitemap_change_frequency) {
                $sitemap_change_frequency = get_option(['section' => 'hm_seo', 'key' => 'content_sitemap_change_frequency', 'default_value' => 'daily']);
            }
            if ($sitemap_priority == 'auto' OR !$sitemap_priority) {
                $sitemap_priority = get_option(['section' => 'hm_seo', 'key' => 'content_sitemap_priority', 'default_value' => '0.6']);
            }
        ...
        case 'taxonomy':
            ...
            if ($sitemap_change_frequency == 'auto' OR !$sitemap_change_frequency) {
                $sitemap_change_frequency = get_option(['section' => 'hm_seo', 'key' => 'taxonomy_sitemap_change_frequency', 'default_value' => 'daily']);
            }
            if ($sitemap_priority == 'auto' OR !$sitemap_priority) {
                $sitemap_priority = get_option(['section' => 'hm_seo', 'key' => 'taxonomy_sitemap_priority', 'default_value' => '0.8']);
            }
            break;
    }
