	...
	if($content_key!=''){
		...
		if( $hmdb->HasRecords() ){
			while( $row=$hmdb->Row() ){
				...
				echo '<span class="bg-success filter_group_taxonomy" data-key="'.$content_key.'" data-id="'.$row->id.'" data-slug="'.$row->slug.'" data-label="'.$row->name.'">';
...
function ajaxfilter_group_taxonomy_option_list($group_id = 0){
	...
	if(is_numeric($group_id) AND $group_id!='0'){
		...
		if( $hmdb->HasRecords() ){
			...
			echo '<span data-id="'.$group_id.'" data-key="'.$content_key.'" class="btn btn-warning filter_option_taxonomy_value_submit">Thêm lựa chọn</span>';
