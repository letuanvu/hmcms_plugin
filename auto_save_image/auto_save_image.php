...
if(!isset_media_group($args)){
	...
	$whereArray=array('folder'=>MySQL::SQLValue($folder),'parent'=>MySQL::SQLValue(0, MySQL::SQLVALUE_NUMBER));
				...
				if(file_exists($saved_file)){
					...
					if ($handle->uploaded) {
						...
						if ($handle->processed) {
							...
							$media_group = get_option( array('section'=>'auto_save_image','key'=>'auto_save_dir_id') );
